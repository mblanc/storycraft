...
export async function editVideo(
  ...
  try {
    ...
    const fileContent = localPaths.map((path) => `file '${path}'`).join("\n");
    fs.writeFileSync(concatenationList, fileContent);

    const writtenFileContent = fs.readFileSync(concatenationList, "utf8"); // 'utf8' for text files
    ...
    if (USE_SIGNED_URL) {
      ...
      if (withVoiceOver) {
        ...
        const vttDestinationPath = path.join(
          GCS_VIDEOS_STORAGE_URI.replace(`gs://${bucketName}/`, ""),
          vttFileName
        );
        await bucket.upload(path.join(publicDir, vttFileName), {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg.ffprobe(videoInputPath, (err, metadata) => {
      ...
      if (!videoStream || !videoStream.width || !videoStream.height) {
        return reject(
          new Error("Could not determine video dimensions or stream not found")
        );
      }
...
export async function mixAudioWithVoiceovers(
  ...
  try {
    ...
    filterComplex.push(
      `${allStreamsToMix.join("")}amix=inputs=${
        allStreamsToMix.length
...
export async function concatenateMusicWithFade(
  ...
  return new Promise<Buffer>(async (resolve, reject) => {
    ...
    try {
      ...
      if (musicDuration <= 0) {
        return reject(
          new Error("Could not determine music duration or duration is zero.")
        );
      }
    } catch (error) {
      return reject(
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      ffmpeg.ffprobe(tempFilePath, (err, data) => {
        ...
        reject(
          new Error("Could not determine duration from ffprobe metadata.")
        );
      });
    } catch (error) {
